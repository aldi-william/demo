<script setup lang="ts">
    ...
    import logo_biru from '../assets/images/logo_biru.png';
...
<template>
  ...
  <div class="w-full h-full bg-secondary py-20 absolute" :class="isAnimated ? 'hidden':''">
    <img :src="logo_biru" alt="tav logo" class="w-40 mx-auto" preload/>
  ...
  <div class="hidden sm:block w-full h-full flex flex-col justify-center layer pt-12 bg-slate-300" :class="isAbsolute ? 'absolute':''">  
  ...
  <div class="block sm:hidden w-full h-full flex flex-col justify-center layer2 -top-10 bg-slate-300" :class="isAbsolute ? 'absolute':''">
     ...
     <div class="mx-auto sm:ml-auto sm:mr-64 w-11/12 sm:w-3/12 md:w-3/12 lg:w-3/12 xl:w-3/12 2xl:w-3/12 p-8 rounded-xl">
          ...
          <h1 class="text-center text-3xl font-bold mb-2 text-white">SELAMAT DATANG</h1>
          <p class="text-center mb-2 text-lg text-white">Platform LELANG MENANG Mobil Bekas Berkualitas</p>
          <div class="w-full mx-auto">
              <p class="text-left text-white">Username</p>
              <input placeholder="Masukan No. WhatsApp" class="bg-white px-2 py-1 rounded border border-black w-full" v-model="dataLogin.no_whatsapp" type="number"/>
          </div>
     
          <div class="w-full mx-auto my-4">
            <p class="text-left text-white">Password</p>
            <input placeholder="Masukan Password" class="bg-white px-2 py-1 rounded border border-black w-full" v-model="dataLogin.password" :type="passwordFieldType"/>
            <div class="relative" v-show="isAnimated2">
              <button @click="switchVisibility()" class="absolute -top-[25px] right-4"><img :src="mata_password" class="w-6 h-4"></button>
            </div>
            
          </div>
     
          <div class="w-full mx-auto mb-2 flex flex-col sm:flex-row justify-between">
            <div>
              <input type="checkbox"/>
              <span class="mx-2 text-white">Ingat Password Saya</span>
            </div>   
          </div>
          <button class="bg-blue-500 text-white px-2 py-1 w-full mx-auto rounded" @click="logintodashboard()">Masuk</button>
          <div class="flex justify-center mt-1">
            <a href="#" class="text-white" @click="showModal = true">Lupa Password ?</a>
          </div>
